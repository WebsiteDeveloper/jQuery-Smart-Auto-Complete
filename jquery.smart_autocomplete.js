...
(function ($) {
    ...
    $.fn.smartAutoComplete = function () {
        ...
        if (arguments.length < 1) {
            ...
            return $(this[0]).data("smart-autocomplete");
        }

        var default_filter_matcher = function (term, source, context) {
                var matchRegExp = new RegExp(term.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "i");

                return $.grep(source, function (value) {
                    return matchRegExp.test(value);
                });
            },
            default_options = {
                minCharLimit: 1,
                ...
                filter: function (term, source) {
                    ...
                    if ($.type(source) === "array") {
                        return default_filter_matcher(term, source, context);
        ...
        $.event.special.keyIn = {
            ...
            _default: function (ev) {
                var context = ev.target,
                    ...
                    maxChars = (options.maxCharLimit > 0 ?  options.maxCharLimit : Number.POSITIVE_INFINITY),
                    query = ev.smartAutocompleteData.query;
        ...
        $.event.special.resultsReady = {
            ...
            _default: function (ev) {
                var context = ev.target,
                    options = $(context).data("smart-autocomplete"),
                    results = ev.smartAutocompleteData.results;
        ...
        $.event.special.showResults = {
            ...
            _default: function (ev) {
                var context = ev.target,
                    options = $(context).data("smart-autocomplete"),
                    results_container = $(options.resultsContainer),
                    raw_results = ev.smartAutocompleteData.results;
        ...
        $.event.special.noResults = {
            ...
            _default: function (ev) {
                var context = ev.target,
                    options = $(context).data("smart-autocomplete"),
                    result_container = $(options.resultsContainer);
        ...
        $.event.special.itemSelect = {
            ...
            _default: function (ev) {
                var context = ev.target,
                    options = $(context).data("smart-autocomplete"),
                    selected_item = ev.smartAutocompleteData.item,
                    selected_value = $(selected_item).text() || $(selected_item).val();
        ...
        return this.each(function (i) {
            ...
            if (options.resultsContainer === undefined) {
            ...
            $(document).bind("focusin click", function (ev) {
                if (options.autocompleteFocused) {
                    ...
                    if (ev.target === options.resultsContainer[0] || ev.target === options.context || elemIsParent) {
                        return;
                    }
